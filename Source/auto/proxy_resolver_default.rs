...
impl<O:IsA<ProxyResolverDefault>> ProxyResolverDefaultExt for O {
	fn set_gproxy_resolver<P:IsA<gio::ProxyResolver>>(
		...
		unsafe {
			glib::gobject_ffi::g_object_set_property(
				...
				b"gproxy-resolver\0".as_ptr() as *const _,
				gproxy_resolver.to_value().to_glib_none().0,
			);
		}
	}
	...
	fn connect_gproxy_resolver_notify<F:Fn(&Self) + 'static>(
		...
		unsafe extern fn notify_gproxy_resolver_trampoline<
			...
			F:Fn(&P) + 'static,
		>(
			this:*mut ffi::SoupProxyResolverDefault,
		...
		unsafe {
			...
			connect_raw(
				...
				b"notify::gproxy-resolver\0".as_ptr() as *const _,
				Some(transmute::<_, unsafe extern fn()>(
					notify_gproxy_resolver_trampoline::<Self, F> as *const (),
				)),
				Box_::into_raw(f),
			)
		}
