...
use crate::{WebsocketConnectionType, WebsocketState, URI};
use glib::{
	object::{Cast, IsA},
...
pub trait WebsocketConnectionExt: 'static {
	...
	fn connect_max_incoming_payload_size_notify<F: Fn(&Self) + 'static>(
		&self,
		f: F,
	) -> SignalHandlerId;
...
impl<O: IsA<WebsocketConnection>> WebsocketConnectionExt for O {
	...
	fn max_incoming_payload_size(&self) -> u64 {
		unsafe {
			ffi::soup_websocket_connection_get_max_incoming_payload_size(
				self.as_ref().to_glib_none().0,
			)
		}
	}
	...
	fn send_text(&self, text: &str) {
		unsafe {
			ffi::soup_websocket_connection_send_text(
				self.as_ref().to_glib_none().0,
				text.to_glib_none().0,
			);
		}
	...
	fn set_keepalive_interval(&self, interval: u32) {
		unsafe {
			ffi::soup_websocket_connection_set_keepalive_interval(
				self.as_ref().to_glib_none().0,
				interval,
			);
		}
		...
		unsafe {
			...
			connect_raw(
				...
				Some(transmute::<_, unsafe extern fn()>(
					closing_trampoline::<Self, F> as *const (),
				)),
				Box_::into_raw(f),
			)
		}
	...
	fn connect_max_incoming_payload_size_notify<F: Fn(&Self) + 'static>(
		...
		unsafe extern fn notify_max_incoming_payload_size_trampoline<
			P: IsA<WebsocketConnection>,
			F: Fn(&P) + 'static,
		>(
			this: *mut ffi::SoupWebsocketConnection,
